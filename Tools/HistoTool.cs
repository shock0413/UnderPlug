...
namespace Tools
{
    public partial class HistoTool : UserControl, ToolInterface, IDisposable
    {
        ...
        public HistoTool(bool isCreateControl)
        {
        ...
        /// <param name="isCreateControl">컨트롤 사용여부</param>
        public HistoTool(string tool, bool isCreateControl)
        {
        ...
        public int Run()
        {
            ...
            double histo_value = m_Histo.Find_Histo(m_Histo.Image, m_Histo.Region);

            if (m_IsCreateControl == true)
                lb_HistoValue.Text = histo_value.ToString();

            if (histo_value >= (double)m_MinValue && (double)m_MaxValue >= histo_value)
                ret = 0;
            else
                ret = 1;
            ...
            if (_isDisplay)
            {
                m_Histo.DrawLabel("Histo Value: " + histo_value, m_Display, m_Histo.CenterX, m_Histo.CenterY, 10, ret == 0 ? CogColorConstants.Green : CogColorConstants.Red, CogColorConstants.Black);
