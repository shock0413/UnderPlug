...
namespace Owf.Controls
{
	public partial class DigitalDisplayControl : UserControl
	{
        ...
        private string _formatText = "00000000";
        [Browsable(true), DefaultValue("00000000")]
        public string Format
        {
            get { return _formatText; }
            set { _formatText = value; Invalidate(); }
        }
		...
		private void DigitalGauge_Paint(object sender, PaintEventArgs e)
		{
            ...
            _digitText = string.Format("{0:" + _formatText + "}", _value);
			...
			{
				using (SolidBrush lightBrush = new SolidBrush(Color.FromArgb(40, _digitColor)))
				{
					sevenSegmentHelper.DrawDigits(
						_digitText, font, brush, lightBrush,
						(ClientSize.Width - digitSizeF.Width),
	...
	public class SevenSegmentHelper
	{
		...
		{
            int cnt = 0;
            for (; cnt < text.Length-1; cnt++)
            {
                if (Char.IsDigit(text[cnt]) && text[cnt] - '0' == 0)
