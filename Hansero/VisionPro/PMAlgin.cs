...
namespace Hansero.VisionLib.VisionPro
{
	...
	public class PMAlgin : ToolBase
	{
        ...
        public double FindPattern(Cognex.VisionPro.Display.CogDisplay Display, bool bViewArea)
        {
            ...
            m_CogPMAlignTool.InputImage = (CogImage8Grey)m_Image;
            #endregion
            ...
            try
            {
                for (int i = 0; i < m_Pattern.Count; i++)
                {
                    ...
                    m_CogPMAlignTool.Pattern = (CogPMAlignPattern)m_Pattern[i];
		...
		public void Set_Pattern(string Name)
		{
            ...
            //트레인 이미지 자르기
            ICogImage image = m_PatternTool.Pattern.TrainImage;
            CogCopyRegionTool regionTool = new CogCopyRegionTool();
            regionTool.InputImage = m_Image;
            regionTool.Region = m_PatternArea;
            regionTool.Run();
            m_PatternTool.Pattern.TrainImage = regionTool.OutputImage;

            //마스크 이미지 자르기
            regionTool.InputImage = m_MaskImage;
            regionTool.Region = m_PatternArea;
            regionTool.Run();
            m_PatternTool.Pattern.TrainImageMask = (CogImage8Grey)regionTool.OutputImage;

            m_MaskImage = (CogImage8Grey)regionTool.OutputImage;
        ...
        /// <param name="PointUse">포인트 사용 유무</param>
        public void Set_Pattern(string Name, ICogImage Image, bool PointUse)
        {
            CogPMAlignTool m_PatternTool = new CogPMAlignTool();
            // 캡춰된 이미지 설정
            if (m_Image == null)
            {
                m_Image = new CogImage8Grey(Image.ToBitmap());
            }
            ...
            if (m_MaskImage != null)
            {
                m_MaskImage.CoordinateSpaceTree = m_Image.CoordinateSpaceTree;
                m_MaskImage.SelectedSpaceName = m_Image.SelectedSpaceName;
                regionTool.InputImage = m_MaskImage;
                regionTool.Region = m_PatternArea;
                regionTool.Run();
                m_PatternTool.Pattern.TrainImageMask = (CogImage8Grey)regionTool.OutputImage;
